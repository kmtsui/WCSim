...
// IWCD with mPMTs, updated geometry as of 20230630, plusu OD with scintillator
void WCSimDetectorConstruction::SetIWCD_WithODScintil_Geometry()
{
  WCDetectorName = "IWCD_mPMT_WithODScintil";
  WCIDCollectionName = WCDetectorName +"-glassFaceWCPMT";
	mPMT_ID_PMT = "PMT3inchR12199_02";    //can be changed in macro through mPMT settings.
	mPMT_OD_PMT = "PMT3inchR12199_02";
  WCSimPMTObject * PMT = CreatePMTObject(mPMT_ID_PMT, WCIDCollectionName);
  ...
  // OD Dimensions //
  WCODLateralWaterDepth    = 5.*cm;
  WCODHeightWaterDepth     = 5.*cm;
  WCODDeadSpace            = 200.*mm;
  WCODTyvekSheetThickness  = 1.*mm; // Quite standard I guess
  WCODScintilPlatesLength = 50*cm;
  WCODScintilPlatesThickness = 7*mm;
  WCODDiameter             = WCIDDiameter + 2*(WCBlackSheetThickness+WCODDeadSpace+WCODTyvekSheetThickness+WCODScintilPlatesThickness);

  // OD PMTs // Placeholder for SiPM
  WCODCollectionName = WCDetectorName + "-glassFaceWCPMT_OD";
  WCSimPMTObject *PMTOD = CreatePMTObject("PMT3inchR14374", WCODCollectionName);
  WCPMTODName           = PMTOD->GetPMTName();
  WCPMTODExposeHeight   = PMTOD->GetExposeHeight();
